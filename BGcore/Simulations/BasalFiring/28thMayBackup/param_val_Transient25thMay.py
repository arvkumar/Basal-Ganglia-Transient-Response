...
def get_parameters():
	...
	p.ctxEndInputtime = 700.0 #In msec
	p.runtime = 700.0 #In msec
	p.leftTimeWindow = 100.0
	p.rightTimeWindow = 250.
	...
	p.jstnTI = 0.300 #0.175 
	p.jTIstn = -0.04*2*2 #- 2nd APR

	p.jTIGPI = -35.0 #-18.0 # Changed Ani -76.0 #-35.0 #-18.0 #(King) # changed -35.0, to bring down GPI at higher inputs - 2nd APR
	p.jstnGPI = 1.91 #0.91 #0.45 #0.91 #
	p.jd1GPI = -12.5 #-13.5-1.5 #-12.0 #-3.0 #-1.0 - 2nd APR
	...
	p.beta_V_thD1 = 0.05 # Computed with "V_th": p.thD1=-45 # 0.0296
	p.beta_E_LD1 = -0.1147 # Computed with E_L = -87.2, at dop=0, it should be 10mv lesser # 0.0296
	...
	p.stimulationStartPoints = [random.randint(300, 400) for i in range(p.NUMTRIALS)] #[200 for i in range(p.NUMTRIALS)] #[random.randint(300, 400) for i in range(p.NUMTRIALS)]
