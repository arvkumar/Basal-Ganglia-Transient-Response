...
def resetDopConnectionWeightsSelective(source, target):
	#Restore synweights
	if source =="None" and target == "None":
		print("+++++++++++++++++++++++++++ It is PD state++++++++++++++++++++++++++++++++")
	else:
		if source == "TATIAll":
			#restore source to target syn strength
			param.staticsyn["GPTA"]["GPTA"] = param.staticsynBase["GPTA"]["GPTA"]
			param.staticsyn["GPTA"]["GPTI"] = param.staticsynBase["GPTA"]["GPTI"]
			param.staticsyn["GPTI"]["GPTA"] = param.staticsynBase["GPTI"]["GPTA"]
			param.staticsyn["GPTI"]["GPTI"] = param.staticsynBase["GPTI"]["GPTI"]
			#restore bg noise

			param.staticsynNoise["GPTI"] = 	param.staticsynNoiseBase["GPTI"]
			param.staticsynNoise["GPTA"] = 	param.staticsynNoiseBase["GPTA"]

			#reset connection prob
			param.cparam["GPTA"]["GPTA"] = param.cparamBase["GPTA"]["GPTA"]
			param.cparam["GPTA"]["GPTI"] = param.cparamBase["GPTA"]["GPTI"]
			param.cparam["GPTI"]["GPTA"] = param.cparamBase["GPTI"]["GPTA"]
			param.cparam["GPTI"]["GPTI"] = param.cparamBase["GPTI"]["GPTI"]

			param.nparam["GPTI"]["tau_syn_in"] = param.nparamBase["GPTI"]["tau_syn_in"]
			param.nparam["GPTI"]["tau_syn_ex"] = param.nparamBase["GPTI"]["tau_syn_ex"]
			param.nparam["GPTA"]["tau_syn_in"] = param.nparamBase["GPTA"]["tau_syn_in"]
			param.nparam["GPTA"]["tau_syn_ex"] = param.nparamBase["GPTA"]["tau_syn_ex"]

		elif source == "STNTILOOP":
			#restore source to target syn strength
			param.staticsyn["GPTI"]["STN"] = param.staticsynBase["GPTI"]["STN"]
			param.staticsyn["STN"]["GPTI"] = param.staticsynBase["STN"]["GPTI"]
			#restore bg noise
			param.staticsynNoise["GPTI"] = 	param.staticsynNoiseBase["GPTI"]
			param.staticsynNoise["STN"] = 	param.staticsynNoiseBase["STN"]
			#reset connection prob
			param.cparam["GPTI"]["STN"] = param.cparamBase["GPTI"]["STN"]
			param.cparam["STN"]["GPTI"] = param.cparamBase["STN"]["GPTI"]
			param.nparam["STN"]["tau_syn_in"] = param.nparamBase["STN"]["tau_syn_in"]
			param.nparam["STN"]["tau_syn_ex"] = param.nparamBase["STN"]["tau_syn_ex"]
			param.nparam["GPTI"]["tau_syn_in"] = param.nparamBase["GPTI"]["tau_syn_in"]
			param.nparam["GPTI"]["tau_syn_ex"] = param.nparamBase["GPTI"]["tau_syn_ex"]

		else:
			#restore source to target syn strength
			param.staticsyn[target][source] = param.staticsynBase[target][source]
			#restore bg noise

			param.staticsynNoise[target] = 	param.staticsynNoiseBase[target]
			param.staticsynNoise[source] = 	param.staticsynNoiseBase[source]

			#reset connection prob
			param.cparam[target][source] = param.cparamBase[target][source]

			param.nparam[target]["tau_syn_in"] = param.nparamBase[target]["tau_syn_in"]
			param.nparam[target]["tau_syn_ex"] = param.nparamBase[target]["tau_syn_ex"]

			

		if (source=="TATIAll" and target=="None"):
			param.noise['GPTA']['rate'] = param.noise['GPTA']['rate'] - 160 + 20
			param.noise['GPTI']['rate'] = 160.0 + 30. 
			param.noise['GPI']['rate'] = 650.0 + 1875. -  350 + 610 


		elif (source=="GPTI" and target=="FSN"):
			param.noise['FSN']['rate'] = param.noise['FSN']['rate'] + 40
			param.noise['GPTI']['rate'] = param.noise['GPTI']['rate'] + 450. + 400.	
			param.noise['GPI']['rate'] = 620. + 2100.
	

		elif (source=="D2"and target=="GPTI"):
			param.noise['D2']['rate'] = 1950. -30 - 55 -10	
			param.noise['GPTI']['rate'] = 130.0 + 40. + 20. -13
			param.noise['GPI']['rate'] = 650.0 + 2200. + 400 - 350 + 175

		
		elif (source=="D1" and target=="GPI"):
			param.noise['D1']['rate'] = 2330. + 75.	+ 15 - 55
			param.noise['GPI']['rate'] = 680.0
			param.nparam['GPI']['I_e'] = -330. - 170. - 20 	-30. -650 	


		elif (source=="STNTILOOP" and target=="None"):
			param.noise['GPTI']['rate'] = 235.0 + 10 -100 + 80
			param.noise['STN']['rate'] = param.noise['STN']['rate'] + 850. -40 -50 + 110
			param.noise['GPI']['rate'] = 1100. + 2000. - 275 - 100 - 1700 + 105 + 400 + 1500
...
for disconnect in disconnections: #disconnections Not required as of now
	...
	for testCase in totalTestCases:	#totalTestCases Not required as of now	
		...
		dopeffect= 0.0 # Maximum PD condition
		...
		print ('-------------connchanges, connchangeSource, connchangeTarget are ', connchanges, connchangeSource, connchangeTarget, '--------------' ) 
		resetDopConnectionWeightsSelective(connchangeSource, connchangeTarget)


		with open("result_16thApril.csv", 'a', newline='') as f:
			writer = csv.writer(f)
			writer.writerow(["dopLevel=", dopeffect, connchangeSource, connchangeTarget])
		...
		for percentNRecruitedForTrans in [50]: #, 20, 50, 90]: #(10, 101, 10): # % population used for the transient stimulation	
			...
			for numTrials in range(0, param.p.NUMTRIALS, 1):				
				...
				RESULTSFolder = "/home/hw1036849/Documents/kingshuk/TCSBG/GitReleaseEneuro/BGcore/Simulations/Templates/GDFS/" + PDState +"Trial" + str(numTrials)
				...
				with open(IMAGEFOLDERPATH + 'dop_'+str(dopeffect)+'_N='+str(percentNRecruitedForTrans)+'percnt_' + str(StimSeg_1Dur) + 'ms_' + str(StimSeg_2Dur) + 'ms_' + str(StimSeg_3Dur) + 'ms' + '_SingleTrial_GPI_FR.csv','a', newline='') as f:
					writer = csv.writer(f)
					writer.writerow(gpi_trial_heights[-1])

				with open(IMAGEFOLDERPATH + 'dop_'+str(dopeffect)+'_N='+str(percentNRecruitedForTrans)+'percnt_' + str(StimSeg_1Dur) + 'ms_' + str(StimSeg_2Dur) + 'ms_' + str(StimSeg_3Dur) + 'ms' + '_SingleTrialTimestamp_GPI_FR.csv','a', newline='') as f1:
			...
			fig.savefig(IMAGEFOLDERPATH+'dop_'+CTXPopSz+'_'+str(dopeffect)+'_MultiTrial_'+str(param.p.NUMTRIALS)+'_GPI'+'_'+str(outFileName)+'_N='+str(percentNRecruitedForTrans)+'percnt_' + str(StimSeg_1Dur) + 'ms_' + str(StimSeg_2Dur) + 'ms_' + str(StimSeg_3Dur) + 'ms' + '.png')

			with open(IMAGEFOLDERPATH+'dop_'+CTXPopSz+'_'+str(dopeffect)+'_MultiTrial_'+str(param.p.NUMTRIALS)+ '_GPI'+'_'+str(outFileName)+'_N='+str(percentNRecruitedForTrans)+'percnt_' + str(StimSeg_1Dur) + 'ms_' + str(StimSeg_2Dur) + 'ms_' + str(StimSeg_3Dur) + 'ms' + '.csv', 'w', newline='') as f:
				writer = csv.writer(f)
