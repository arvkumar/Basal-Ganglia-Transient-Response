function syn_range_trial_Exp()
%UNTITLED4 Summary of this function goes here
%   Detailed explanation goes here

source_file='\Figure5_images\';

dest_folder='\Figure_5output\';

dest_folder1= dest_folder;

mkdir (dest_folder1)
basal_window=99;
stimulation_Start=100;
Time_limit=stimulation_Start+150;
...
%%late excitation and late inhibtion detection
        ...
        third_zone_Data=[];
          ...
          %if (size(temp_Result_in,2)>1)
          if (size(temp_Result_in,2)>1)&((temp_Result_in{1,2}(1)-temp_Result_in{1,1}(2))<20)         
          for jjj=1:2%size(temp_Result_in,2)
              ...
              %for j=1:1%size(temp_Result_in,2)
                fprintf(fp2,' %s ', num2str(temp_Result_in1{1,1})); 
                %% to make it equal
                fprintf(fp2,' %s %s', num2str(0),num2str(0));
             %end
          end
           for jjj=1:3%size(area_zone,2)
                if(length(area_zone1{1,j})~=0)
...
[area_update_ad,zone_update_ad,mean_ad,std_ad,max_min_Zone]=zone_cal_test(stimulation_Start,basal_window,Time_limit,area_update,zone_update,zone_mean2,zone_std2,dest_folder,source_file); 
 num='method';
 
 zone_update_ad(find (zone_update_ad==-100))= 0;
 zone_update_ad(find (zone_update_ad==1))= 0;
 
dlmwrite('area.txt',area_update_ad)
dlmwrite('duration.txt', zone_update_ad)
...
% %         saveas(f,dest_path) ;
% dest_path=strcat(dest_folder,'\', num2str(folder_plot_name),'late.png');
%         saveas(f,dest_path) ;
...
% 
        ...
        sub_dest_path=horzcat(dest_folder,folder_plot_name);
        mkdir(sub_dest_path)
        dest_path=strcat(sub_dest_path,'\', file_plot_name,'early.png');
        saveas(f,dest_path) ;
